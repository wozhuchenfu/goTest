...
/**
挖矿：
回到加密电子货币中，比特币就是通过让参与者利用这样的加密算法求解出符合特定条件的哈希值来实现“挖矿”过程。
具体来说，比特币要求参与者通过 double SHA-256 算法计算出“前导0”超过若干位的哈希值，
第一个求解出来的参与者就是“获胜的矿工”。
 */
...
func generateBlock(oldBlock Block,BPM int) (Block,error) {
	...
	for i :=0; ;i++  {
		...
		newBlock.Nonce = hex
		fmt.Println(hex)
		if !isHashValid(calculateHaash(newBlock),newBlock.Difficulty){
			fmt.Println(calculateHaash(newBlock),"do more work!")
			time.Sleep(time.Second)
			continue
		} else {
			fmt.Println(calculateHaash(newBlock),"work done!")
